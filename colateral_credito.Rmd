...
# Consulta adicional
# valores únicos de la columna CIIU

consulta <- paste0(
  "SELECT DISTINCT CIIU ",
  "FROM ", tabla
)

df_ciiu_unicos <- sqlQuery(ch, consulta)

# CAST para forzar CIIU como varchar
consulta <- paste0(
  "SELECT CAST(CIIU AS VARCHAR(10)) AS CIIU, ",
  "CASE WHEN DIAS_DE_MORA >= 30 THEN 1 ELSE 0 END AS EN_MORA, ",
  "COUNT(*) AS N_REG ",
   "FROM ", tabla, " ",
...
df_ciiu <- sqlQuery(ch, consulta) # esta arroja 908 mil observaciones. Además el CIIU que resulta tiene decimales. Hay que revisar que la variable CIIU si sean códigos CIIU (que deben ser mmuuuchos menos)
...
# no calculo el ratio de entrada para poder tener las magnitudes en pesos
